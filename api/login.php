<?php
    ...
    if (isset($_POST['email']) && isset($_POST['password'])) {
        ...
        $user = "SELECT user.id,user.status,user.is_deleted,user.password, user.name, user.phone, user.email, user.image, user.firebase_id, role.name AS role FROM user LEFT OUTER JOIN user_role ON user_role.user_id = user.id LEFT OUTER JOIN role ON role.id = user_role.role_id WHERE user.email = '$email'";
                ...
                if($row_user['is_deleted'] == 1){
                    ...
                    if($row_user['status'] == 0){
                        ...
                        if($row_user['password'] == $password){
                            ...
                            $response["role_name"] = $row_user['role'];
                            header('Content-Type: application/json');
                            echo json_encode($response);
