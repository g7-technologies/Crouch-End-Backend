<?php
    ...
    if (isset($_POST['post_id']) && isset($_POST['user_id']) && isset($_POST['comment_id']) && isset($_POST['comment_reply'])) {
        ...
        if($result_user){
            ...
            if( mysqli_num_rows($result_user) > 0 ){
                ...
                if($row_user['is_deleted'] == 1){
                    ...
                    if($row_user['status'] == 0){
                        ...
                        $create_comment = "INSERT INTO comment_reply (user_id,comment_id,reply,status) VALUES ('$user_id','$comment_id','$comment_reply',1)";
                        $result_create_comment = mysqli_query($conn,$create_comment);
                        
                        if($result_create_comment){
                            
                            $response["error"] = FALSE;
                            $response["success_msg"] = "reply Created Successfully!";
                            ...
                            $statement = $user_nam." replied you on your comment";
                            if($q3 != $user_id){
                                $q4 = mysqli_query($conn,"INSERT INTO notifications (notification_from,user_id,notification,comment_id,post_id) VALUES ('$user_id','$q3','$statement','$comment_id',$post_id)");
                            }
                            header('Content-Type: application/json');
                            echo json_encode($response);
                        }
            ...
            $response["error"] = TRUE;
            $response["error_msg"] = "Unable to comment..! Try Again Later";
            header('Content-Type: application/json');
            echo json_encode($response);
        }
    }
